...
state("Dolphin")
{
	/* Offset to the addresses (0x80000000-0x8040FFFF) FOR HEROES 32 BIT is address - 0x10000 on 4.0 */
	/* On 5.0 Pure, RAM starts at 0x7FFF0000 */
	...
	int gameID: "Dolphin.exe", 0xDCDFD8, 0x0; // First int of memory, should be 0x45533947 ('ES9G' in little endian or 'G9SE' in big endian) if game is loaded
	byte emblemScreen: "Dolphin.exe", 0xDCDFD8, 0x42C28F; // 1 when game is in emblem screen (igt paused)
	byte lastByteOfGFC: "Dolphin.exe", 0xDCDFD8, 0x452C4F; // To check if to start the splitting
}
...
init
{
	...
	vars.GFC1 = new DeepPointer("Dolphin.exe", 0xDCDFD8, 0x452C4C);
	vars.GFC2 = new DeepPointer("Dolphin.exe", 0xDCDFD8, 0x452C4D);
	vars.GFC3 = new DeepPointer("Dolphin.exe", 0xDCDFD8, 0x452C4E);
	vars.GFC4 = new DeepPointer("Dolphin.exe", 0xDCDFD8, 0x452C4F);
}
